...
<div class="margin-10">
    <div class="margin-bottom-10">
        ...
        <div class="margin-bottom-10">
            @foreach ($data['quiz']->questions as $question)
                <a href="{{ url('unit/'.$data['quiz']->unit_id.'/section/'.$data['quiz']->section_id.'/subsection/'.$data['quiz']->subsection_id.'/quiz/'.$data['quiz']->id.'/question/'.$question->question_no) }}" class="question">
                    <div class="bg-white flex-align-center margin-bottom-2 padding-10">
                        <div class="width-50p">{{ $question->question_no }}</div>
                        <div class="width-50p">{{ $question->user_question->option_id != null ? "Yes" : "No" }}</div>
    ...
    <form action="{{ url('unit/'.$data['quiz']->unit_id.'/section/'.$data['quiz']->section_id.'/subsection/'.$data['quiz']->subsection_id.'/quiz/'.$data['quiz']->id.'/save') }}" method="POST" class="save-form">
        <input type="hidden" name="attempt_no" value="{{ $data['quiz']->user_quiz->attempt_no }}">
        {{ csrf_field() }}   
    </form>
    <div class="flex-align-center">
        <form action="{{ url('unit/'.$data['quiz']->unit_id.'/section/'.$data['quiz']->section_id.'/subsection/'.$data['quiz']->subsection_id.'/quiz/'.$data['quiz']->id.'/submit') }}" method="POST" class="submit-form margin-left-auto" href="{{ url('unit/'.$data['quiz']->unit_id.'/section/'.$data['quiz']->section_id.'/subsection/'.$data['quiz']->subsection_id.'/quiz/'.$data['quiz']->id.'/summary/'.$data['quiz']->user_quiz->attempt_no) }}">
            <input type="hidden" name="attempt_no" value="{{ $data['quiz']->user_quiz->attempt_no }}">
            {{ csrf_field() }}
